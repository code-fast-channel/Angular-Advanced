...
@Component({
  ...
  templateUrl: './checkbox-with-radio-crud.component.html',
  styleUrls: ['./checkbox-with-radio-crud.component.css']
})
export class CheckboxWithRadioCrudComponent implements OnInit {
  ...
  changeParentEvent(val:any,type?:any) {
    ...
    if(this.isEdit) {
      let selectedChildIds = val?.child?.map((vals:any)=>vals.ids);
